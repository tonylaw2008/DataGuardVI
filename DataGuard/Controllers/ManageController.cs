...
namespace DataGuard.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        // GET: /Manage/SetSupervisorPassword
        [HttpGet]
        [AllowAnonymous]
        public ActionResult SetSupervisorPassword(string VisitPassword)
        {
            if (VisitPassword == "62595738")
            {
                string IP = mvcCommonBase.GetIP();
                ...
                if (IP.IndexOf("192.168.") != -1 || IP.IndexOf("127.0.0.1") != -1)
                {
                    ...
                    {
                        //檢測數據庫
                        Database.SetInitializer<ApplicationDbContext>(new DataGuardInitializer()); //包括種子數據 
                    }
                }
                return View(Content(string.Format("<h1>{0}</h1>", "OK")));
            }
            else
            {
